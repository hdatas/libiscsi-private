...
struct iscsi_pdu *iscsi_allocate_pdu(struct iscsi_context *iscsi, enum iscsi_opcode opcode,
...
int iscsi_process_target_nop_in(struct iscsi_context *iscsi, struct iscsi_in_pdu *in) {
  ...
  if (ttt == 0xffffffff) return 0;

  iscsi_send_target_nop_out(iscsi, ttt, lun, slu);
  return 0;
}

static void iscsi_reconnect_after_logout(struct iscsi_context *iscsi, int status, void *command_data _U_, void *opaque _U_) {
  if (status) {
...
int iscsi_process_reject(struct iscsi_context *iscsi, struct iscsi_in_pdu *in) {
...
void iscsi_pdu_set_lun(struct iscsi_pdu *pdu, uint32_t lun, uint64_t slu) {
  ...
  uint64_t temp = lun;
  temp = (temp << 48) | slu;
  scsi_set_uint64(&pdu->outdata.data[8], temp);
}
