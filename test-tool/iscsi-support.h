...
#define WRITE10(...)                                                        \
  ({                                                                        \
     ...
     if (_r == -2) {                                                        \
       ...
       CU_PASS("[SKIPPED] Target does not support WRITE10. Skipping test"); \
       return;                                                              \
     }                                                                      \
     CU_ASSERT_EQUAL(_r, 0);                                                \
     _r;                                                                    \
...
static inline int pr_type_is_all_registrants(enum scsi_persistent_out_type pr_type) {
  switch (pr_type) {
...
int prout_register_and_ignore(struct scsi_device *sdev, unsigned long long key);
...
int prout_preempt(struct scsi_device *sdev, unsigned long long sark, unsigned long long rk,
    enum scsi_persistent_out_type pr_type);
...
int compareandwrite(struct scsi_device *sdev, uint64_t lba, unsigned char *data, uint32_t len, int blocksize,
    int wrprotect, int dpo, int fua, int group_number, int status, enum scsi_sense_key key, int *ascq, int num_ascq);
...
int modesense6(struct scsi_device *sdev, struct scsi_task **task, int dbd, enum scsi_modesense_page_control pc,
    enum scsi_modesense_page_code page_code, int sub_page_code, unsigned char alloc_len, int status, enum scsi_sense_key key, int *ascq, int num_ascq);
...
int orwrite(struct scsi_device *sdev, uint64_t lba, uint32_t datalen, int blocksize,
    int wrprotect, int dpo, int fua, int fua_nv, int group, unsigned char *data, int status, enum scsi_sense_key key, int *ascq, int num_ascq);
...
int readdefectdata12(struct scsi_device *sdev, struct scsi_task **task,
    ...
    int status, enum scsi_sense_key key,
    int *ascq, int num_ascq);
int report_supported_opcodes(struct scsi_device *sdev, struct scsi_task **save_task, int rctd, int options,
    int opcode, int sa, int alloc_len, int status, enum scsi_sense_key key, int *ascq, int num_ascq);
...
int receive_copy_results(struct scsi_task **task, struct scsi_device *sdev, enum scsi_copy_results_sa sa, int list_id, void **datap,
