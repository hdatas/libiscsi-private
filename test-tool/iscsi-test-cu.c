...
static void print_usage(void) {
  ...
  fprintf(stderr, "  -I|--initiator-name-2=iqn-name   2nd Initiatorname to use [%s]\n", initiatorname2);
  fprintf(stderr, "  -t|--test=test-name-reg-exp      Test(s) to run [ALL] <NOT YET IMPLEMENTED>\n");
  ...
  fprintf(stderr, "Where <iscsi-url> iSCSI URL format is: %s\n\n", ISCSI_URL_SYNTAX);
  fprintf(stderr, "<host> is one of:\n");
  fprintf(stderr, "  \"hostname\"       e.g. iscsi.example\n");
  fprintf(stderr, "  \"ipv4-address\"   e.g. 10.1.1.27\n");
  fprintf(stderr, "  \"ipv6-address\"   e.g. [fce0::1]\n\n");
  fprintf(stderr, "and <test-name-reg-exp> is of the form: FAMILY[.SUITE[.TEST]]\n\n");
}
...
int main(int argc, char *argv[]) {
  ...
  if ((mp_num_sds == 0) || (mp_sds[0]->iscsi_url == NULL && mp_sds[0]->sgio_dev == NULL)) {
#ifdef HAVE_SG_IO
  ...
  for (i = 0; i < mp_num_sds; i++) {
    ...
    if (res < 0) {
      fprintf(stderr, "Failed to connect to SCSI device %d\n", i);
      goto err_sds_free;
    }
  }
