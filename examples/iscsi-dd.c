...
int populate_seg_desc_hdr(unsigned char *hdr, int dc, int cat, int src_index, int dst_index) {
  int desc_len = 28;
...
void populate_param_header(unsigned char *buf, int list_id, int str,
    int list_id_usage, int prio, int tgt_desc_len, int seg_desc_len, int inline_data_len) {
  buf[0] = list_id;
...
void cscd_ident_inq(struct iscsi_context *iscsi, int lun,
...
void cscd_param_check(struct iscsi_context *iscsi, int lun, uint32_t blocksize) {
  struct scsi_task *task = NULL;
...
void readcap(struct iscsi_context *iscsi, int lun, uint64_t slu, int use_16,
  ...
  printf("%s: _blocksize=%d, _num_blocks=%lu\n", __FUNCTION__, *_blocksize, *_num_blocks);
  scsi_free_scsi_task(task);
  return;
}
...
int main(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "6b:d:hi:m:nps:x", long_options, &option_index)) != -1) {
    char *endptr;
    ...
    switch (c) {
      ...
      case 'd':  // dst
        ...
        break;
      case 'h':  // dst
        printf("Ex.\niscsi-dd -s iscsi://172.16.1.56:3260/iqn.2016-01.com.hcd:pool-1-0/255:123 -d iscsi://172.16.1.56:3260/iqn.2016-01.com.hcd:pool-2-0/255:123\n");
        return 0;
      case 'i':  // initiator-name
        initiator = optarg;
        break;
      case 'm':  // max
  ...
  if (iscsi_url == NULL) {
    fprintf(stderr, "Invalid src URL: %s\n", iscsi_get_error(client.src_iscsi));
    exit(10);
  }
  ...
  if (client.use_xcopy) {
    cscd_ident_inq(client.src_iscsi, client.src_lun, &client.src_tgt_desig);
    cscd_param_check(client.src_iscsi, client.src_lun, client.src_blocksize);
  }
  ...
  if (iscsi_url == NULL) {
    fprintf(stderr, "Invalid dst URL: %s\n", iscsi_get_error(client.dst_iscsi));
    exit(10);
  }
  ...
  if (client.use_xcopy) {
    cscd_ident_inq(client.dst_iscsi, client.dst_lun, &client.dst_tgt_desig);
    cscd_param_check(client.dst_iscsi, client.dst_lun, client.dst_blocksize);
  }
